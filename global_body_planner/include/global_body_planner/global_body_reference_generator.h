...
//! A global body reference generator for legged robots
/*!
   GlobalBodyReferenceGenerator is a container for all of the logic utilized in the global
   body reference generator node. This algorithm requires the initial and final robot state as input,
   and will publish the global body plan as a BodyPlan message over a topic.
   It will also publish the discrete states used by the planner (from which the full path is interpolated).
*/

class GlobalBodyReferenceGenerator {
public:
    /**
     ...
     */
    GlobalBodyReferenceGenerator(ros::NodeHandle nh);
